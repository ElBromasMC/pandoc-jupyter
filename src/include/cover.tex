...
\draw[ultra thick,gray] ($(current page.center)+(5,0)$) -- ++(0,5cm) node[midway,left=0.25cm,text width=15cm, align=right, black!75]{{\fontsize{25}{40} \selectfont \textbf{UNIVERSIDAD  NACIONAL  MAYOR  DE  SAN  MARCOS} \\[10pt] }} node[midway,right=0.25cm,text width=6cm,align=left,black]{{\begin{figure}[H]
    \includegraphics[width=4cm]{./src/include/logo.png}
\end{figure}}};

\node at ($(current page.center)+(0,-2)$) {{\fontsize{36}{72} \selectfont Técnicas de Modelamiento}};
\node at ($(current page.center)+(0,-4)$) {{\fontsize{24}{72} \selectfont Tarea}};
\node at ($(current page.center)+(0,-5)$) {{\fontsize{20}{72} \selectfont Clasificación de los puntos criticos}};

\node[text width=15cm,align=center] at ($(current page.center)+(0,-8)$) {{\fontsize{16}{19.2} \selectfont \textcolor{black}{ \textbf{Linares Rojas, Ander Rafael}\\ \vspace{1em}Código: 22140367}}};
